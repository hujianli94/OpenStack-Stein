...
|----------------------------|------------------------------------------------------------|----------------------------|----------------------------|------------------|
| aodh                  | 提供告警服务，用于监控和告警系统                                 | Pecan+WSME                      | SQLAlchemy + Alembic           |                  |
| barbican              | 提供密钥管理服务，用于管理加密密钥                              | Pecan+WSME                       | SQLAlchemy + Alembic                     |                  |
| blazar                | 提供资源预订管理服务，支持云资源的预订                          | v1: Flask；v2: Pecan+WSME                       | SQLAlchemy + Alembic |                  |
| ceilometer           | 提供计量和监控服务，用于收集资源使用数据                        | 无                      | sqlalchemy-migrate           |                  |
| cinder               | 提供块存储服务，管理虚拟机的磁盘存储                           | Paste+webob+routes          | sqlalchemy-migrate |                  |
| cloudkitty            | 提供云资源计费系统，用于管理计费数据                           | Pecan+WSME                      | SQLAlchemy + Alembic |                  |
...
| ironic               | 提供裸金属服务，支持物理服务器的管理                           | Pecan+WSME                      | SQLAlchemy + Alembic |                  |
...
| magnum                | 提供容器管理服务，用于管理 Kubernetes 和 Docker 集群             | Pecan+WSME                  | SQLAlchemy + Alembic |                  |
...
| mistral               | 提供工作流引擎服务，支持自动化任务管理                         | Pecan+WSME                  | SQLAlchemy + Alembic |                  |
...
| octavia               | 提供负载均衡服务，支持 Layer 4 和 Layer 7 负载均衡                | Pecan+WSME                  | SQLAlchemy + Alembic |                  |
| openstack-congress    | 提供策略和合规性服务，支持 OpenStack 环境的策略定义和管理      | Paste+webob+routes    | sqlalchemy-migrate           |                  |
| openstack-cyborg      | 提供硬件加速服务，支持 FPGA 等硬件资源的管理                   | Pecan+WSME                  | SQLAlchemy + Alembic |                  |
...
| panko                 | 提供事件监控和存储服务，支持 OpenStack 环境中的事件处理        | Pecan+WSME                  | SQLAlchemy + Alembic |                  |
| python-watcher        | 提供自动化监控和资源管理服务                                   | Pecan+WSME                      | sqlalchemy-migrate           |                  |
| qinling               | 提供函数计算服务，支持无服务器计算框架                         | Pecan+WSME                  | SQLAlchemy + Alembic |                  |
...
| solum                 | 提供云应用生命周期管理服务                                     | Pecan+WSME                  | SQLAlchemy + Alembic |                  |
| storlets              | 提供存储服务扩展，支持存储的插件式开发                         | 无独立API（嵌入Swift）       | 无（依赖Swift） |                  |
| swift                | 提供对象存储服务，支持海量数据的存储                           | Paste+webob+routes          | SQLAlchemy + Alembic |                  |
| tacker                | 提供网络功能虚拟化（NFV）管理服务                              | Pecan+WSME                  | SQLAlchemy + Alembic |                  |
| tricircle            | 提供跨多个数据中心的网络服务                                   | Pecan+WSME | sqlalchemy-migrate |                  |
| trove                | 提供数据库服务，支持多种数据库管理                              | Paste+webob+routes     | sqlalchemy-migrate |                  |
| vitrage               | 提供智能监控和分析服务，支持资源的异常检测和预测               | Pecan+WSME                  | SQLAlchemy + Alembic |                  |
| zaqar                 | 提供消息队列服务，用于处理异步消息                             | Paste+webob+routes     | SQLAlchemy + Alembic |                  |
| zun                   | 提供容器管理服务，支持容器生命周期管理                         | Pecan+WSME                  | SQLAlchemy + Alembic |                  |
...
fork：
- https://github.com/bopopescu/OpenStack-Stein.git
- https://github.com/hujianli94/OpenStack-Stein.git
